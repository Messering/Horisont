...
public class WavePath {
    ...
    public static Vector<Unit.Action> getPath(Cell[][] my_map, int x, int y, int nx,
                                         int ny) {
        ...
        Vector<Unit.Action> path = new Vector<Unit.Action>();
        int cx = nx+1, cy = ny+1;
        int step = map[cx][cy];
        while (map[cx][cy]!=0) {
            ...
            for (int k = 0; k <= 1 && !finded; k++) {
                if (map[cx + 1 - 2 * k][cy] == step - 1) {
                    finded = true;
                    if (k == 0) {
                        path.add(Unit.Action.left);
                        cx++;
                    }
                    if (k == 1) {
                        path.add(Unit.Action.right);
                        cx--;
                    }
                    ...
                    if (map[cx][cy + 1 - 2 * k] == step - 1) {
                        finded = true;
                        if (k == 0) {
                            path.add(Unit.Action.up);
                            cy++;
                        }
                        if (k == 1) {
                            path.add(Unit.Action.down);
                            cy--;
                        }
                        //System.out.println(path.get(path.size() - 1));
                    }
